...
class BaseWriteForm(forms.ModelForm):
    ...
    def clean_recipients(self):
        ...
        if exchange_filter:
            ...
            if filtered_names:
                errors.append(
                    self.error_messages['filtered'].format(users=', '.join(filtered_names)))
            if errors:
    ...
    def save(self, recipient=None, parent=None, auto_moderators=[]):
        ...
        for r in recipients:
            ...
            # force_insert=True is not accessible from here
    ...
    # commit_on_success() is deprecated in Django 1.6 and will be removed in
    # Django 1.8
    save = transaction.atomic(save) if hasattr(
        transaction, 'atomic') else transaction.commit_on_success(save)


class WriteForm(BaseWriteForm):

    """The form for an authenticated user, to compose a message."""
    # specify help_text only to avoid the possible default 'Enter text to
    # search.' of ajax_select v1.2.5
...
class AnonymousWriteForm(BaseWriteForm):
    ...
    # The features available to anonymous users should be kept to the strict
    # minimum.
    can_overwrite_limits = False

    email = forms.EmailField(label=_("Email"))
    recipients = CommaSeparatedUserField(label=(
        _("Recipients"), _("Recipient")), help_text='', max=1)  # one recipient is enough
